...
export interface ConditionalKeyShare {
  validator: string;
  condition: string;
  keyShare: string;
  keyShareIndex: number;
...
function createBaseConditionalKeyShare(): ConditionalKeyShare {
  return { validator: "", condition: "", keyShare: "", keyShareIndex: 0, receivedTimestamp: 0, receivedBlockHeight: 0 };
}

export const ConditionalKeyShare = {
  encode(message: ConditionalKeyShare, writer: _m0.Writer = _m0.Writer.create()): _m0.Writer {
    if (message.validator !== "") {
      writer.uint32(10).string(message.validator);
    }
  ...
  decode(input: _m0.Reader | Uint8Array, length?: number): ConditionalKeyShare {
    const reader = input instanceof _m0.Reader ? input : new _m0.Reader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = createBaseConditionalKeyShare();
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
        case 1:
  ...
  fromJSON(object: any): ConditionalKeyShare {
    return {
      validator: isSet(object.validator) ? String(object.validator) : "",
  ...
  toJSON(message: ConditionalKeyShare): unknown {
    const obj: any = {};
    message.validator !== undefined && (obj.validator = message.validator);
  ...
  fromPartial<I extends Exact<DeepPartial<ConditionalKeyShare>, I>>(object: I): ConditionalKeyShare {
    const message = createBaseConditionalKeyShare();
    message.validator = object.validator ?? "";
